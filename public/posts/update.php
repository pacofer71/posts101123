<?php
session_start();
//si no estoy logeado error de autorizacion y salimos
if(!isset($_SESSION['id'])){
    header("HTTP/1.1 401 Unauthorized");
    require __DIR__."/../errores/error401.htm";
    die();
}
//estoy logeado pero no mando el id del post a actualizar, lo mando a index
if(!isset($_GET['id'])){
    header("Location:index.php");
    die();
}

$postId=$_GET['id']; //id del post a modificar
...
//Comprobemos que efectivamente este post pertenece a este usuario
$post=Post::detalle($postId, $userId); //si no lo encuentra devuelve false y si no los datos del post
if(!$post){
    header("HTTP/1.1 401 Unauthorized");
    require __DIR__."/../errores/error401.htm";
    die();
    ...
    if (is_uploaded_file($_FILES['imagen']['tmp_name'])) {
        if (!Utilidades::imagenValida($_FILES['imagen']['type'], $_FILES['imagen']['size'])) {
            ...
            if (!move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta . $imagen)) {
                ...
                //borramos la imagen anterior del post salvo que sea la imagen por defecto
...
<body style="background-color:blanchedalmond">
    <div class="container p-8 mx-auto">
        ...
        <div class="w-3/4 mx-auto p-6 rounded-xl bg-gray-400">
            <form method="POST" action="<?php echo $_SERVER['PHP_SELF']."?id=$postId"    ?>" enctype="multipart/form-data">
                <div class="mb-6">
                    ...
                    id="nombre" placeholder="Título..." class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <?php
                ...
                <div class="mb-6">
                    <div class="flex w-full">
                        ...
                        <div class="w-1/2">
                            <img src="<?php echo "./../".$post->imagen; ?>" class="h-72 rounded w-full object-cover border-4 border-black" id="img">
                        </div>
                    </div>
